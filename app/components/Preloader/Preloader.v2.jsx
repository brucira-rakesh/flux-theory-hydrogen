 ...
 * Reveal is 1s (was 2s); the overlay then fades out in EXIT_MS.
 */
const BOOT_DURATION_SEC = 1;
 ...
 * every registered asset (three.js + `sequence` + `additionalSequence`) has
 * loaded. `sequence` also supplies the boot plate's still image (frame 0);
 * `additionalSequence` is gate-only — use it for a sequence that needs to be
 * fully preloaded before reveal but shouldn't be painted as that still image
 * (e.g. Seawave's frames, wired in from HomeV2Page — see
 * PreloaderContext's `additionalSequenceLoader` comment).
 */
export const PreloaderV2 = ({
  ...
  sequence,
  additionalSequence,
  // Floor for the still image's reveal tween (0..1) — the still is visible
  // at this opacity from the very first paint (before the tween even
  // starts, and for however long asset-loading takes ahead of it) instead
  // of sitting fully invisible, then ramps up to fully opaque by handoff.
  // Defaults to 0 (the original behavior) so existing callers are unaffected.
  stillMinOpacity = 0,
  // Overlays the still image with the same wet-glass droplet effect used on
  // the hero itself (see RainOverlay) — off by default for existing callers.
  showStillRain = false,
}) => {
  ...
  const [additionalSequenceReady, setAdditionalSequenceReady] = useState(
    () => !additionalSequence,
  );
  const [progress, setProgress] = useState(0);

  const stillRef = useRef(null);
  // Both live ABOVE the droplet overlay — see applyReveal for why the reveal
  // can't be carried by the still image itself once the rain is up.
  ...
  const recomputeProgress = useCallback(() => {
    ...
    const totalWeight = s.managerTotal + s.seqTotal + s.seq2Total;
    setProgress(
      totalWeight > 0
        ? Math.min(
            1,
            Math.max(
              0,
              (s.managerLoaded + s.seqLoaded + s.seq2Loaded) / totalWeight,
            ),
          )
        : 1,
    );
  ...
  const applyReveal = useCallback(
    (t) => {
      ...
      // RainOverlay's canvas is an OPAQUE WebGL context (`alpha: false`, see
      // rainRenderer) that repaints a full-brightness copy of this same
      // image across the whole viewport every frame — and it samples the
      // <img>'s decoded bitmap via drawImage, which ignores CSS opacity
      // entirely. So with the droplets up, dimming the still is invisible:
      // the boot plate jumped to fully revealed the moment the rain
      // initialised, no matter how far along loading actually was.
      //
      // The reveal therefore rides on two layers stacked ABOVE the rain
      // instead — a dark veil for the brightness ramp and the colour tint —
      // and the still just stays opaque as the texture the rain samples.
      if (still) {
        gsap.set(still, {
          opacity: showStillRain
            ? 1
            : stillMinOpacity + (1 - stillMinOpacity) * raw,
        });
      }
      if (!showStillRain) return;

      const veil = veilRef.current;
      // Matches what the still's own floor opacity used to darken to, so the
      // held state looks the same as it did before the rain was added.
      if (veil) gsap.set(veil, { opacity: (1 - raw) * (1 - stillMinOpacity) });
      const tint = tintRef.current;
      if (tint) gsap.set(tint, { opacity: 1 - raw });
    },
    [stillMinOpacity, showStillRain],
  );
  ...
  // Boot tween — runs immediately, in parallel with asset loading. The still
  // is already on screen (`preloader__still`, fetchPriority high) so the
  // reveal doesn't need to wait for the reel; finish() below still waits for
  // both the tween AND every registered asset before fading the overlay out.
  ...
  useEffect(() => {
    ...
    sequenceLoader
      .preloadSequence(
        ...
        { readyCount: sequence?.readyCount },
      )
      .then(() => {
  ...
  // Same shape as `sequenceLoader` above, for `additionalSequence` — a second
  // sequence that gates the boot without being shown as the still image (see
  // PreloaderContext's own comment on `additionalSequenceLoader`).
  const additionalSequenceLoader = useMemo(() => {
    if (
      prefersReducedMotion() ||
      !additionalSequence?.frameCount ||
      !additionalSequence?.getFramePath
    ) {
      return null;
    }
    return createSequenceLoader(additionalSequence);
  }, [additionalSequence]);

  useEffect(() => {
    if (!additionalSequenceLoader) return undefined;

    let cancelled = false;
    loadStatsRef.current.seq2Total = additionalSequence?.frameCount || 0;
    recomputeProgress();
    additionalSequenceLoader
      .preloadSequence(
        (_index, ratio) => {
          if (cancelled) return;
          loadStatsRef.current.seq2Loaded = ratio * loadStatsRef.current.seq2Total;
          recomputeProgress();
        },
        { readyCount: additionalSequence?.readyCount },
      )
      .then(() => {
        if (!cancelled) setAdditionalSequenceReady(true);
      });

    return () => {
      cancelled = true;
    };
  }, [additionalSequenceLoader, additionalSequence, recomputeProgress]);

  // Clear the overlay once the boot animation and every registered asset
  // (three.js scaffold + both optional image sequences) have finished.
  useEffect(() => {
    if (animationDone && assetsReady && sequenceReady && additionalSequenceReady)
      finish();
  }, [animationDone, assetsReady, sequenceReady, additionalSequenceReady, finish]);
  ...
  const contextValue = useMemo(
    () => ({
      ...
      progress,
      animationDone,
      assetsReady: assetsReady && sequenceReady && additionalSequenceReady,
      manager,
      sequenceLoader,
      additionalSequenceLoader,
    }),
        ...
        >
          {stillFrameSrc && (
            <img
              ...
              draggable={false}
              decoding="async"
              // Sampled straight into a WebGL texture by RainOverlay
              // (`showStillRain` below) — loaded cross-origin (VITE_CDN_URL)
              // without this, that taints the canvas the same way the other
              // Image()/video crossOrigin fixes address.
              crossOrigin="anonymous"
              // This is the boot screen's only visible content AND the
              // texture the droplet overlay samples (see `showStillRain`),
              // so it must not sit behind the frame-sequence preload in the
              // request queue — that delay is what kept the rain from
              // starting until loading was already finished.
              fetchPriority="high"
              // Inline so the very first paint already shows the floor
              // opacity — the CSS class's own opacity:0 would otherwise be
              // what's visible until the reveal effect below has a chance
              // to run.
              style={{ opacity: stillMinOpacity }}
            />
          )}
          {!showStillRain && (
            <div className="preloader__tint" aria-hidden="true" />
          )}
          {stillFrameSrc && showStillRain && (
            <RainOverlay
              getVideo={() => stillRef.current}
              className="preloader__rain"
            />
          )}
          {/* Above the rain, in this order, so the reveal is actually
              visible through the droplets — see applyReveal. Inline opacity
              so the very first paint is already the held (fully tinted)
              state, ahead of the reveal tween's first frame. */}
          {showStillRain && (
            <>
              <div
                ref={veilRef}
                ...
                aria-hidden="true"
                style={{ opacity: 1 - stillMinOpacity }}
              />
              <div
                ref={tintRef}
                className="preloader__tint preloader__tint--over-rain"
                aria-hidden="true"
                style={{ opacity: 1 }}
              />
            </>
          ...
          <div className="preloader__center" aria-hidden="true">
            <div className="preloader__logo">
              ...
              >
                <defs>
                  {/* Same wet-glass grain filter as IntroHeroV3's title
                      (turbulence -> displace -> blur, see
                      #intro-hero-v2-title-filter) — duplicated under its own
                      id so this loader doesn't depend on that component's
                      DOM being mounted. */}
                  <filter
                    id="preloader-logo-filter"
                    x="-20%"
                    y="-40%"
                    width="140%"
                    height="180%"
                ...
                <path
                  filter="url(#preloader-logo-filter)"
